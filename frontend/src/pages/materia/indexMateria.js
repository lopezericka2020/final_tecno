...
import RowMateria from './rowMateria';

export default function IndexMateria(props) {
    const navigate = useNavigate();
    const { loading, error, data } = useQuery(GET_ALLMATERIA);
                                ...
                                <div className="table-responsive">
                                    <table className="table table-striped">
                                        <tbody>
                                            ...
                                            { data.getAllMateria.map( ( item, key ) => {
                                                return (
                                                    <RowMateria key={key} materia={item} index={key} />
                                                );
                                            } ) }
                                        </tbody>
                                    </table>
