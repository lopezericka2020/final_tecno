...
export default function EditInscripcion(props) {
    ...
    const getInscripcion = useQuery(
        gql`
            {
                getInscripcionDetalle( id: "${params.idinscripcion}" ) {
                    id
                    gestion
                    alumno
                    materia
    ...
    React.useEffect( () => {
        if ( !getInscripcion.loading ) {
            if ( getInscripcion.data ) {
                if ( getInscripcion.data.getInscripcionDetalle ) {
                    setGestion(getInscripcion.data.getInscripcionDetalle.gestion);
                    setAlumno(getInscripcion.data.getInscripcionDetalle.alumno);
                    setMateria(getInscripcion.data.getInscripcionDetalle.materia);
                    setDocente(getInscripcion.data.getInscripcionDetalle.docente);
                    setConcurrencia(getInscripcion.data.getInscripcionDetalle.concurrencia);
                    setEstado(getInscripcion.data.getInscripcionDetalle.estado === "true" ? true : false);
                }
            }
        }
    }, [ getInscripcion ] );

    const [ updateInscripcion ] = useMutation( gql`
        mutation {
            updateInscripcionDetalle (
                id: "${params.idinscripcion}"
                input: {
                    gestion: "${gestion}",
                    alumno: "${alumno}",
                    materia: "${materia}",
    ...
    function onSubmitStore(e) {
        ...
        updateInscripcion().then( (data) => {
            if ( data.data.updateInscripcionDetalle ) {
                if ( data.data.updateInscripcionDetalle.success === true ) {
                    navigate('/inscripcion/index');
                } else {
                    toastr.error( data.data.updateInscripcionDetalle.message, 'Error al actualizar', { closeButton: true, progressBar: true, } );
                }
            }
        } ). catch ( (e) => {
            console.log(e);
    ...
    if ( gestionResponse.loading || alumnoResponse.loading || materiaResponse.loading || docenteResponse.loading || getInscripcion.loading ) {
        return (
    ...
    if ( gestionResponse.error || alumnoResponse.error || materiaResponse.error || docenteResponse.error || getInscripcion.error ) {
        return (
